...
describe('parseServiceStatus', () => {
  it('returns correct state for inactive/failed statuses', () => {
    expect(parseServiceStatus('inactive')).toEqual({ state: 'inactive', color: 'gray', label: 'Offline' });
    expect(parseServiceStatus('failed')).toEqual({ state: 'failed', color: 'orange', label: 'Error' });
  });

  it('returns correct state for active status', () => {
    expect(parseServiceStatus('active')).toEqual({ state: 'active', color: 'green', label: 'Online' });
  });

  it('returns correct state for transitioning statuses', () => {
    expect(parseServiceStatus('activating')).toEqual({ state: 'transitioning', color: 'yellow', label: 'Working...' });
    expect(parseServiceStatus('deactivating')).toEqual({ state: 'transitioning', color: 'yellow', label: 'Working...' });
  ...
  it('returns unknown state for empty/non-string inputs', () => {
    expect(parseServiceStatus('')).toEqual({ state: 'unknown', color: 'orange', label: 'Unknown' });
    expect(parseServiceStatus(null)).toEqual({ state: 'unknown', color: 'orange', label: 'Unknown' });
    expect(parseServiceStatus(undefined)).toEqual({ state: 'unknown', color: 'orange', label: 'Unknown' });
  });
});
